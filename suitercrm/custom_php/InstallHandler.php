...
class InstallHandler extends LegacyHandler
{
     ...
     * @return array
     */
    public function checkDBConnection(array $inputArray): array
    {
        ...
        try {
            ...
            /**
             * Attempt to prevent install to NOT empty DB.
             * @author erick.pham.dev@gmail.com
             */
            // ** ADDITION START **
            // Now, attempt to select the specific database
            $pdo->exec("USE `$dbName`;"); // Use backticks for database names to handle special characters

            // ** ADDITION START: Check if the database is empty **
            $stmt = $pdo->query("SHOW TABLES;");
            if ($stmt) {
                $tableCount = $stmt->rowCount();
                if ($tableCount > 0) {
                    $errorMessage = "The selected database '{$dbName}' is not empty. Please provide an empty database or clear the existing one to proceed.";
                    $this->logger->error('Database not empty: ' . $errorMessage);
                    return ['checked' => false, 'message' => $errorMessage];
                }
            }
            // ** ADDITION END **
            ...
            // return false;
            return ['checked' => false, 'message' => $e->getMessage()];
        }

        // return true;
        return ['checked' => true, 'message' => "Database connection successful and database '{$dbName}' is empty."];
    }
